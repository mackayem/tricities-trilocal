...
if ( ! function_exists( 'understrap_posted_on' ) ) {
	...
	function understrap_posted_on() {
		...
		if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
			...
			$time_string_updated = '<time class="entry-date updated" datetime="%3$s">%4$s</time>';
		}
		$time_string = sprintf(
			$time_string,
		...
		$posted_on   = apply_filters(
			...
			sprintf(
				'<span class="posted-on">%1$s <a href="%2$s" rel="bookmark">%3$s</a></span><br>',
		...
		$updated_on   = apply_filters(
			'understrap_updated_on',
			sprintf(
				'<span class="updated-on">%1$s <a href="%2$s" rel="bookmark">%3$s</a></span><br>',
				esc_html_x( 'Updated on', 'post date', 'understrap' ),
				esc_url( get_permalink() ),
				apply_filters( 'understrap_updated_on_time', $time_string_updated )
			)
		);
		...
		echo $posted_on . $updated_on . $byline; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
		...
		<nav class="container navigation post-navigation">
			...
			<div class="d-flex nav-links justify-content-between">
				...
				<span class="blog-main">
					<a href="<?php echo $archive ?>">Blog Main</a>
				</span>				
				<?php
